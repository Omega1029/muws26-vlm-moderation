...
% >>> Target venue: ICLR 2027 (main track) <<<
...
falls from $\sim$0.95 to $0.36$ (hate) and $0.29$ (sarcasm). Fine-tuning's first-order effect is
to stop the constant-flag collapse and produce a model that actually \emph{abstains} on benign
content---the minimum requirement for a deployable scanner. The adapted flag rate sits
\emph{below} the prevalence of the evaluation slices ($0.50$ hate, $0.42$ sarcasm), i.e.\ the
scanner is now somewhat under-flagging rather than over-flagging; for a deployment that
prefers recall, the operating threshold should be tuned on the score rather than left at $0.5$.
