...
with the model's prediction rate falling from $\sim$0.95 to $0.29$--$0.36$ in both cases.
...
\noindent\textbf{De-biasing first.} On both tasks, \texttt{pred\_pos} collapses from
$\sim$0.95 to $0.36$ (hate) and $0.29$ (sarcasm): fine-tuning's first-order effect is to stop
the constant-answer behaviour. Note that this \emph{overshoots} the evaluation-set base rate
in both cases ($0.50$ hate, $0.42$ sarcasm), so the adapted model is mildly conservative
rather than calibrated---but it is answering the input rather than answering a constant.
